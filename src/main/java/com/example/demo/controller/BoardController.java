...
public class BoardController {
    ...
    public String list(Model model)
        ...
        // 동작하게 설계되어 있다.
        // DB에서 가져온 정보들을 싹다 가져오는 것. (DB에서 읽어온 정보들이 다 list에 있는 것)
        model.addAttribute(
                "list",
                // DB에서 list를 가져오는 정보.
                service.list()
    ...
    public String read(int boardNo, Model model)
        ...
        // read가 빨간색이 나온 이유는 service가 없기 때문!
        // 그렇기 때문에 repository에 가서 해당 정보를 기입해줘야 함.
    ...
    // "수정"을 만든다면
    // 1. 게시판 번호가 필요할 것. (GetMapping)
    @GetMapping("/getModify")
    public String modify(int boardNo, Model model)
                                throws Exception {
        log.info("getModify()");

        // 값 수정을 하려면 내가 수정하려고 하는 정보를 획득해야 하기 때문에 (read해서 한번 읽어오는 것)
        // modify에서 수정을 하고 update해주면 됨.
        model.addAttribute(service.read(boardNo));
    ...
    //한번 읽어온 정보는 게시판에 있는 모든 정보가 들어오기 때문에 board로 받아오는 것.
...
// controller -> repository -> service 타는 형식으로 정보 받아옴.
...
// 2. (실제로 table을 만들었으므로) entity 패키지에 실제 사용할 Entity 클래스를 작성한다.
// 3. controller 패키지에 URL을 처리할 ItemManiaController를 만든다.
// 4. @Service에 대한 인터페이스와 구현체를 작성한다.
// 5. @Repository에 대한 DB 처리를 실제로 수행한다.
// 6. 각각의 필요한 HTML 파일들을 작성한다.
// 7. HTML 연동 순서
// 7-1. list(모든 게시글을 확인할 수 있는 형태)
// 7-2. regoster(게시글을 등록할 수 있도록 만든다.)
